...
public sealed class TerminalControlTests : AvaloniaTestBase
{
    ...
    public Task DragSelection_UpdatesSelectedTextAndControlBinding()
    {
        return RunInHeadlessSession(() =>
        {
            ...
            Assert.Equal(TerminalSamples.SelectionSampleText[..8], model.SelectedText);
    ...
    public Task DoubleClickSelection_SelectsWord()
    {
        return RunInHeadlessSession(() =>
        {
            ...
            int selectionStart = TerminalSamples.SelectionSampleText.IndexOf("selection", StringComparison.Ordinal);
            control.HandleSelectionPressed(control.GetCellCenter(selectionStart + 1, 0), KeyModifiers.None, clickCount: 2);
            control.HandleSelectionReleased(control.GetCellCenter(selectionStart + 1, 0), KeyModifiers.None, clickCount: 2);

            Assert.True(model.HasSelection);
            Assert.Equal("sel", model.SelectedText);
        });
    }
    ...
    public Task RightClickAction_CopyOrPaste_CopiesSelection()
    {
        return RunInHeadlessSession(() =>
        {
            ...
            Assert.Equal(TerminalSamples.SelectionSampleText[..8], clipboardText);
